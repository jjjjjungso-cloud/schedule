...
with tab1:
    ...
    if st.button("🚀 데이터 통합 정제 시작"):
        ...
        st.success("✅ 정제 완료! 3단계 분석 탭으로 이동하세요.")
...
with tab3:
    if not st.session_state.df_master.empty:
        ...
        with s2:
            ...
            p_ward = df.pivot_table(index='성함', columns=df['날짜'].dt.day, values='계획병동', aggfunc='first')
            p_shift = df.pivot_table(index='성함', columns=df['날짜'].dt.day, values='계획근무조', aggfunc='first')
        ...
        with s3:
            ...
            a_ward = df.pivot_table(index='성함', columns=df['날짜'].dt.day, values='실제병동', aggfunc='first')
            a_shift = df.pivot_table(index='성함', columns=df['날짜'].dt.day, values='실제근무조', aggfunc='first')
            st.dataframe(pd.concat([a_ward, a_shift], keys=['병동', '근무조']).sort_index(), use_container_width=True)
    else:
        st.info("2단계에서 데이터 정제를 먼저 실행해주세요.")
