...
# --- 메인 UI ---
st.set_page_config(page_title="프라임 시스템", layout="wide")
...
tab1, tab2, tab3, tab4 = st.tabs(["📂 1단계: 업로드 & 정제", "📊 3단계: 분석", "🎯 4단계: 배정", "📋 5단계: 결원대체 확인"])
...
with tab3:
    if not st.session_state.df_master.empty:
        ...
        with s2:
            ...
            st.dataframe(pd.concat([plan_ward, plan_shift], keys=['병동', '근무조']).sort_index(), use_container_width=True)
        with s3:
            act_ward = df.pivot_table(index='성함', columns=df['날짜'].dt.day, values='실제병동', aggfunc='first')
            act_shift = df.pivot_table(index='성함', columns=df['날짜'].dt.day, values='실제근무조', aggfunc='first')
            st.dataframe(pd.concat([act_ward, act_shift], keys=['병동', '근무조']).sort_index(), use_container_width=True)
...
with tab5:
    # 5단계 분석 로직
    st.header("📋 결원대체(비상 투입) 이력 확인")
    df_sub = st.session_state.df_master[st.session_state.df_master['계획병동'] != st.session_state.df_master['실제병동']].dropna(subset=['실제병동'])
    st.dataframe(df_sub[['날짜', '성함', '계획병동', '실제병동', '실제근무조']], use_container_width=True)
