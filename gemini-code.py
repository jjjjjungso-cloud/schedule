...
def expand_generic_data(df):
    """배정표(Plan) 및 요청(Request) 데이터를 평일 단위로 분리"""
    expanded_list = []
...
# --- 메인 UI ---
st.set_page_config(page_title="프라임 인재 육성 & 배정 플랫폼", layout="wide")
st.title("🏥 프라임 간호 인재 육성 & 통합 배정 플랫폼")
...
with tab1:
    ...
    file_rep = col3.file_uploader("3. 대체간호사 내역 (System)", type=["xlsx", "csv"])
...
with tab2:
    if file_p and file_a and file_rep and file_r:
        if st.button("🚀 3단 교차검증 및 데이터 정제 시작"):
            ...
            st.success("✅ 교차 검증 완료! 3~5단계 탭을 이용하세요.")
...
with tab3:
    if not st.session_state.df_master.empty:
        ...
        s1, s2 = st.tabs(["📊 1. 병동별 누적 경험치 집계", "🚨 2. 매칭 추적 (화이트박스)"])
        
        with s1:
            st.subheader("💡 간호사별 육성 현황 (지원 vs 결원 분리)")
            ...
            rep_counts = nurse_df[nurse_df['실제역할'].isin(['결원', '⚠️긴급변경'])].groupby('실제병동').size().reset_index(name='결원 횟수(독립수행)')
            
            summary_res = pd.merge(sup_counts, rep_counts, on='실제병동', how='outer').fillna(0)
            summary_res.rename(columns={'실제병동': '병동'}, inplace=True)
            if not summary_res.empty:
                summary_res['지원 횟수(워밍업)'] = summary_res['지원 횟수(워밍업)'].astype(int)
                summary_res['결원 횟수(독립수행)'] = summary_res['결원 횟수(독립수행)'].astype(int)
            st.dataframe(summary_res, use_container_width=True)

        with s2:
            st.subheader("🔍 일별 매칭 추적표 (이력 더블체크)")
            def tracking_flow(row):
                ...
                elif role == '⚠️긴급변경': return f"{p} ➔ {a}(⚠️긴급)"
                else: return f"{p} ➔ {a}"
                
            df['매칭흐름'] = df.apply(tracking_flow, axis=1)
            pivot_compare = df.pivot_table(index='성함', columns=df['날짜'].dt.day, values='매칭흐름', aggfunc='first').reindex(index=all_nurses, columns=all_days).fillna("")
...
with tab4:
    if not st.session_state.df_master.empty and not st.session_state.df_req_next.empty:
        ...
        st.header("🚨 결원 발생 ➔ 최적 투입 간호사 마스터 랭킹")
        st.info("💡 **[병동 중심]** 당일 결원이 발생했을 때, 프라임 팀 전체 인력 중 누구를 투입하는 것이 환자 안전과 간호사 성장에 가장 좋은지 추천합니다.")
        ...
        if st.button("🔍 전 간호사 마스터 랭킹 가동", type="primary"):
            ...
            for nurse in all_prime_nurses:
                ...
                if sup_count > 0 and rep_count == 0:
                    priority_tag, desc, score = "1순위", "🚀 실전 투입 최적기 (지원만 O)", 1
                elif sup_count > 0 and rep_count > 0:
                    priority_tag, desc, score = "2순위", "✅ 검증된 안정 자원 (둘 다 O)", 2
                elif sup_count == 0 and rep_count > 0:
                    priority_tag, desc, score = "3순위", "⚠️ 하드랜딩 경험자 (결원만 O)", 3
                else:
                    priority_tag, desc, score = "4순위", "🌱 결원 파견 보류 (경험 전무)", 4
                    
                recommend_list.append({
                    "간호사 성함": nurse, "소속": NURSE_TO_BLD.get(nurse, "기타"),
                    f"[{target_ward}병동] 지원 횟수": sup_count, f"[{target_ward}병동] 결원 횟수": rep_count,
                    "우선순위 그룹": priority_tag, "상태 진단": desc, "총 결원 피로도": total_rep, "_score": score
                })
                
            df_rec = pd.DataFrame(recommend_list)
            df_rec.sort_values(by=['_score', f'[{target_ward}병동] 지원 횟수', '총 결원 피로도'], ascending=[True, True, True], inplace=True)
            df_rec.drop(columns=['_score'], inplace=True)
            df_rec.reset_index(drop=True, inplace=True)
            df_rec.index += 1
            
            st.dataframe(df_rec, use_container_width=True)
    else: st.info("데이터가 준비되지 않았습니다.")
...
with tab5:
    if not st.session_state.df_master.empty and not st.session_state.df_req_next.empty:
        ...
        with col_logic:
            ...
            hist_list = get_recent_history_list(df_master, selected_nurse, week_info['날짜'].min() if not week_info.empty else datetime.today())
            rec_shift = recommend_shift_logic(hist_list)
            st.success(f"💡 연속 근무 피로도 방지를 위한 추천 근무조: **{rec_shift}**")
        with col_select:
            final_shift = st.radio("배정 근무조 선택", ["D", "E"], index=0 if rec_shift == "D" else 1, horizontal=True)

        st.divider()
        st.subheader(f"🏥 {selected_nurse} 간호사 역량 맞춤형 '대기 병동' 추천")
        allow_switch = st.checkbox("🚩 소속 외 타 동(Building) 스위치 허용")
        ...
        if not avail_today.empty:
            ...
            for w in avail_today['계획병동'].unique():
                ...
                if c_sup > 0 and c_rep == 0: 
                    priority = "2순위 (워밍업 중, 결원 대비 대기)"
                    score = 2
                elif c_sup == 0 and c_rep == 0: 
                    priority = "1순위 (새로운 병동 인큐베이팅 필수!)"
                    score = 1
                
                recommend_list.append({
                    "요청 병동": w, "소속 동": WARD_TO_BLD.get(w, "기타"), 
                    "기존 지원 횟수": c_sup, "기존 결원 횟수": c_rep,
                    "대기 배정 추천도": priority,
                    "_score": score
                })
                
            if recommend_list:
                res_df = pd.DataFrame(recommend_list).sort_values(by=["_score", "기존 지원 횟수"])
                res_df.drop(columns=['_score'], inplace=True)
                res_df.reset_index(drop=True, inplace=True)
                res_df.index += 1
                
                st.dataframe(res_df, use_container_width=True)
            else: st.warning("해당 주차/근무조에 조건에 맞는 병동 요청이 없습니다.")
        else: st.error("해당 주차에 선택한 근무조의 병동 지원 요청건이 없습니다.")
