...
def clean_actual_data(uploaded_file, year, month_int):
    """[팀장님 지정 룰] 실제 근무표 정제: 슬래시(/) 기준 완벽 추출"""
    ...
    for sheet_name in xl.sheet_names:
        ...
        for _, row in df.iterrows():
            ...
            for d_idx in day_cols:
                ...
                # 규칙 1: 셀 내용이 빈칸이거나 딱 '/' 하나만 있으면 패스
                if code in ['NAN', 'NONE', '', '/']:
                    continue
                
                # 규칙 2: / (슬래시)가 있는지 먼저 찾는다. (없으면 병가, 보건 등으로 무시)
                if '/' not in code:
                    continue
                ...
                # 규칙 3: 슬래시 앞부분(Front)에서 D 또는 E가 있는지 확인하여 근무조 확정
                shift = 'E' if 'E' in front_part else 'D'
                
                # 규칙 4: 슬래시 뒷부분(Back)에서 숫자가 있으면 그 숫자를 실제 근무 병동으로 확정
                ward_nums = re.findall(r'\d+', back_part)
                if not ward_nums:
                    continue # 숫자가 아예 없으면 무시
...
with tab2:
    if file_p and file_a and file_r:
        if st.button("🚀 데이터 통합 정제 시작"):
            st.session_state.df_master = pd.DataFrame() # 초기화
            try:
                ...
                if not df_p.empty and not df_a.empty:
                    ...
                    st.error("🚨 데이터 파싱 실패: 파일 양식이나 규칙에 맞는 데이터가 없습니다.")
...
with tab5:
    if not st.session_state.df_master.empty and '계획병동' in st.session_state.df_master.columns:
        ...
        if '실제병동' not in df_m.columns:
            st.error("🚨 2단계에서 실제 근무표의 병동 데이터가 합쳐지지 않았습니다. 양식을 확인해 주세요.")
        else:
            # 안전한 비교를 위해 계획병동과 실제병동에서 숫자만 추출
            df_m['계획_비교'] = df_m['계획병동'].astype(str).str.extract(r'(\d+)')[0].fillna('')
            df_m['실제_비교'] = df_m['실제병동'].astype(str).str.extract(r'(\d+)')[0].fillna('')
            
            # 결원대체 조건: 실제 병동 데이터가 있고, 계획 숫자와 실제 숫자가 다를 때!
            df_sub = df_m[
                (df_m['실제_비교'] != '') & 
