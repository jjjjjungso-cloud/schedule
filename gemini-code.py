...
def get_strategic_report(unit):
    ...
    for _, n in nurses.iterrows():
        ...
        recs.append({
            ...
            "차기 추천 병동": recommend, "상태": "⭐신규공략" if recommend in not_visited else "숙련도유지"
        })
    conn.close()
...
st.title("🏥 프라임 간호사 스마트 운영 시스템")
st.sidebar.header("🔍 설정 및 단계 선택")
if st.sidebar.button("⚙️ 간호사 13명 초기 등록"):
    register_initial_nurses()
    st.sidebar.success("등록 완료!")

step = st.sidebar.radio("작업 선택", ["1. 배정표(Plan) 검증", "2. 근무표(Actual) 검증", "3. 통합 비교 및 전략 분석"])
...
if step == "1. 배정표(Plan) 검증":
    ...
    if up_p:
        ...
        else: st.warning("데이터를 찾지 못했습니다.")

# --- Step 2: 근무표 검증 ---
elif step == "2. 근무표(Actual) 검증":
    st.header("📅 실제 근무표(Actual) 정제")
    up_a = st.file_uploader("근무표 업로드", type="xlsx")
    sel_month = st.sidebar.selectbox("해당 월", [f"{i}월" for i in range(1, 13)])
...
# --- Step 3: 통합 비교 및 전략 분석 ---
elif step == "3. 통합 비교 및 전략 분석":
    st.header("⚖️ 계획 vs 실제 통합 리포트")
    c1, c2 = st.columns(2)
    with c1: up_p = st.file_uploader("1. 배정표(Plan)", type="xlsx", key="p3")
    with c2: up_a = st.file_uploader("2. 근무표(Actual)", type="xlsx", key="a3")
    sel_month = st.sidebar.selectbox("월 선택", [f"{i}월" for i in range(1, 13)], key="m3")
    month_int = int(re.findall(r'\d+', sel_month)[0])
    ...
    if up_p and up_a:
        ...
        if not df_p.empty and not df_a.empty:
            ...
            st.markdown("---")
            st.header("🚀 다음 달 대기 병동 최적화 추천")
            st.write("분석된 병동 경험 이력을 바탕으로 팀의 숙련도를 높일 수 있는 배정을 추천합니다.")
